...
ggplot_gtable.built_lemon <- function(data) {
  ...
  if ('axis_annotation' %in% names(data$plot) &&
    ...
    g <- data$plot$axis_annotation$draw(
        side='right',
        is.primary=get_panel_params(data$layout, 1)$y$position == "right",
    ...
    if (!all(sapply(g$children, is.zero))) {
      ...
      gtable$widths[[gtable$layout$l[i]]] <- Reduce(grid::unit.pmax, widths, gtable$widths[gtable$layout$l[i]])
    }

    #left
    g <- data$plot$axis_annotation$draw(
      side='left',
      is.primary=get_panel_params(data$layout, 1)$y$position == "left",
      range=get_panel_y_range(data$layout, 1),
      theme=plot_theme(data$plot$theme)
    )
    if (!all(sapply(g$children, is.zero))) {
  ...
  if ('axis_annotation' %in% names(data$plot) &&
    ...
    g <- data$plot$axis_annotation$draw(
      side='top',
      is.primary=get_panel_params(data$layout, 1)$x$position == "top",
    ...
    if (!all(sapply(g$children, is.zero))) {
      ...
      gtable$heights[[gtable$layout$l[i]]] <- Reduce(grid::unit.pmax, heights, gtable$heights[gtable$layout$l[i]])
    }

    # bottom
    g <- data$plot$axis_annotation$draw(
      side='bottom',
      is.primary=get_panel_params(data$layout, 1)$x$position == "bottom",
      range=get_panel_x_range(data$layout, 1),
      theme=plot_theme(data$plot$theme)
    )
    if (!all(sapply(g$children, is.zero))) {
